...
class trinityEGIS extends MY_Controller {
   ...
   * COMMENTS: Sidney James Y. Pascasio
	...
	//it gets the data for the subject codes
 ...
 //stores the data of every subject code and description
    public function showSubjectGradesSummaryEGIS() {
		...
		//it records the data. and shows buttons
		...
		if($reportCategory == 1) {
				$results = null;
			//you have to input all the recommended and needed data for it to store and take it to the database
				if($gradingPeriod == '1') {
          ...
          //adds if $q++
				...
				//adds column counts 2 or 3 on said columns
				...
				//gets the data of the tables that are about to be filled
				if($courseCode == '1001') {
          ...
          //a button for you to click to join senior highschool
					$scorePercentage = $this->_getRecordsData($dataSelect = array($selectSP),
				...
				$ptPct = 0;
				$qaPct = 0;
        //grading component code for each row
        ...
        //it gets the records of each data
      ...
      //it results to null if you put the wrong input
				$results = null;
				if($gradingPeriod == '1') {
					$selectFields = "triune_grades_score_sheet_1.*, ";
					$selectFields = $selectFields . "triune_students_k12.studentNumber, concat(triune_students_k12.lastName, ', ', triune_students_k12.firstName, ' ', SUBSTR(triune_students_k12.middleName, 1, 1), '.') as fullName";
					//if you put the wrong input it would result to null
					$results = $this->_getRecordsData($dataSelect = array($selectFields),
						$tables = array('triune_grades_score_sheet_1', 'triune_students_k12'),
						$fieldName = array('sy', 'sectionCode', 'subjectCode' ), $where = array($_SESSION['sy'], $sectionCode, $subjectCode),
						$join = array('triune_grades_score_sheet_1.studentNumber = triune_students_k12.studentNumber'), $joinType = array('inner'),
					...
					$data['scoreList'] = (array) $results;
          //max score counter for each array
					$maxScoreCounter = (array) $results[0];
					$data['maxScoreRow'] = $maxScoreCounter;
				//if the grading period is "one" it would fill said tables
					$titlesData = null;
					if($gradingPeriod == '1') {
						$titlesFields = "triune_grades_score_sheet_1_title.*";
						$titlesData = $this->_getRecordsData($dataSelect = array($titlesFields),
				...
				$data['titles'] = (array) $titlesData[0];
				//loads library pdf or view subject grades summary-2
				$this->load->library('Pdf');
    ...
    public function classCardEGIS() {
      //it views classcard-setup
        //echo "HELLO WORLD";
        $this->load->view('EGIS/classcard-setup');
    }

	//it posts said sectioncode, student number and section codeNS
    public function showClassCardDetailsEGIS() {
		...
		$gradingPeriod = $_SESSION['gP'];
		//input section code, student number and section code NS
		$data['sectionCode'] = $sectionCode;
		$data['studentNumber'] = $studentNumber;
		$data['sectionCodeNS'] = $sectionCodeNS;
		//it stores the records data on the database
		...
		//input data on your first middle last name, gender, year, age and etc.
    ...
    //subtracts section code if null
		$courseCode = substr($sectionCode, 0, 4);
		$selectField1 = null;
		$selectField2 = null;
		...
		$resultsGrades = null;
		if($courseCode == '1002') {
//selects the field if junior high depends on the subject description that you chooses
			...
			$resultsGrades1 = $this->_getRecordsData($dataSelect2 = array($selectField1),
				...
				$fieldNameLike = null, $like = null, $whereSpecial = null, $groupBy = null );
//selects the field depends on the subject description that you chooses
...
//selects the field depends on the subject description that you chooses
    ...
    //it gets the records data and stores its data to the database
    ...
    //it gets the records data and stores its data to the database
      ...
      //it gets the records data and stores its data to the database
		$traits2 = $this->_getRecordsData($dataSelect3 = array('*'),
			...
			$where = array($_SESSION['sy'], $studentNumber, $sectionCode), $join = null, $joinType = null, $sortBy = null, $sortOrder = null,
			$limit = null, $fieldNameLike = null, $like = null, $whereSpecial = null, $groupBy = null );
		//depends on the traits for the array
		...
		//for each grade it either adds or transmute each row
...
//this is for the results grade 2 or row number 2
...
//this shows the ranking by section data
    public function showRankingBySectionEGIS() {
		...
		//this is where it gets the ddata for each table and store it to the database
    ...
    //this is the weight of the array
		$weight = array();
		$this->db->trans_start();
		$subjCtr = 1;
		$weightCtr = 0;
		...
		//where you insert the data if subjCTr is equals to 1
			if($subjCtr == 1) {
				$insertData1 = null;
				$insertData1 = array(
...
//where the data for subjCtr, weightCtr, weight and total weight
		...
		//if grading period is one it stores the ddata and fills in the tables or arrays
		if($gradingPeriod == '1') {
    ...
    //if grading period is one it stores the ddata and fills in the tables or arrays
		foreach($results as $row) {
			...
			$recCtr = 0;
			if(!empty($details)) {
				//it gets the data for scoreCtr=1
